...
\begin{itemize}
\item One n-ary relation where all the $n$ entities in the relation are listed as a row in the table. Each row contains the $n$ identifiers for the $n$ entities, and the attributes associated with entities. This is the case for both the Olist and SBA examples. To create this relation, we typically use the entity of interest, say a loan or an invoice as a pivot and join all the other entities associated with the pivot entity. The attributes associated with the entities are also included in the relation. This is the case when we want to learn a model that predicts something related to the pivot entity. For example, in the SBA example, we want to predict the performance of a loan, so we use the loan as the pivot entity and join all the other entities associated with the loan.
