...
\end{figure}
 ...
 \begin{figure}[H]
\centering
\includegraphics[scale=.35]{good_borrower_loans_7a.png}
...
\item Multiple relations from which we can construct a single n-nary relation.
\end{itemize}

In other words, we can get a dataset $\mathbf{D} = \{ r_1, r_2, \ldots, r_m \}$ where each $r_i$ is a relation. Each relation $r_i$ has a set of entities and attributes associated with the entities. The relations can be used to construct a single n-nary relation $\mathbf{R} = \{ e_1, e_2, \ldots, e_n \}$ where each $e_i$ is an entity and the attributes associated with the entity are the attributes of the relation.
